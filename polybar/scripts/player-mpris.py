...
def _printFlush(status, **kwargs):
    global _last_status
    global scrollCharacterLimit
    if status != _last_status:
        if scrollCharacterLimit != 0 and len(status)>scrollCharacterLimit:
            if timeoutReference != "":
...
def _printScrolling(string, **kwargs):
    ...
    printString = string[currentScrollCharacter:currentScrollCharacter+scrollCharacterLimit]

    if scrollCharacterLimit + currentScrollCharacter  > len(string):
       
        rolloverAmmount =  scrollCharacterLimit + currentScrollCharacter - len(string)  +1
    ...
    if(currentScrollCharacter<len(string)-1):
        ...
        currentScrollCharacter = 1
    timeoutReference = GLib.timeout_add(scrollSpeed, _printScrolling,string)
...
scrollCharacterLimit = int(args.scroll)
scrollSpeed = int(args.scroll_speed)
